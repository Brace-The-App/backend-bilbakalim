...
class QuestionAnswerStatsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(\Spatie\Permission\Middleware\PermissionMiddleware::class . ':view answer statistics')->only(['index', 'showLogs', 'showDetail']);
    ...
    {
        $base = QuestionAnswerStat::query()->where('total_answers', '>', 0);

        $totalAnalyzed = (clone $base)->count();
        $totalAnswers = (clone $base)->sum('total_answers');
        $avgSuccess = round((float) (clone $base)->avg('correct_percentage'), 1);
        ...
        $successBuckets = DB::table('question_answer_stats')
        ...
        // Karttaki uyumsuz sayısı da sadece güvenilir (5+) örneklem
        ...
        $reliableCount = (clone $base)->where('total_answers', '>=', 5)->count();
        $weakCount = (clone $base)->whereBetween('total_answers', [1, 2])->count();
        $mediumConfCount = (clone $base)->whereBetween('total_answers', [3, 4])->count();

        $answerBuckets = DB::table('question_answer_stats')
            ->where('total_answers', '>', 0)
            ->selectRaw("
        ...
        foreach ($mismatchRows as $q) {
            $text = method_exists($q, 'getTranslation')
                ? $q->getTranslation('question', 'tr')
                : (string) $q->question;
            $mismatchList[] = [
                'id' => $q->id,
                'text' => \Illuminate\Support\Str::limit(strip_tags($text), 50),
        ...
        // Grafikte bar görünsün diye min 3 cevap; 1 yanlış = %0 bar uzunluğu 0 oluyordu
        $topHard = Question::query()
            ->join('question_answer_stats as qas', 'questions.id', '=', 'qas.question_id')
        ...
        foreach ($topHard as $q) {
            $text = method_exists($q, 'getTranslation')
                ? $q->getTranslation('question', 'tr')
                : (string) $q->question;
            $labels[] = '#' . $q->id . ' · ' . \Illuminate\Support\Str::limit(strip_tags($text), 28);
        ...
        return [
            'summary' => [
                ...
                'answer_buckets' => [
                    '1' => (int) ($answerBuckets->a1 ?? 0),
                    '2' => (int) ($answerBuckets->a2 ?? 0),
                    '3_4' => (int) ($answerBuckets->a34 ?? 0),
                    '5_plus' => (int) ($answerBuckets->a5 ?? 0),
                ],
            ...
            'success_bands' => [
                'hard' => (int) ($successBuckets->hard_band ?? 0),
                'medium' => (int) ($successBuckets->medium_band ?? 0),
                'easy' => (int) ($successBuckets->easy_band ?? 0),
            ],
            'top_hard' => [
                'labels' => $labels,
                'values' => $values,
